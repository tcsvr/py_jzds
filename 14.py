...
# sql="SELECT `name`,`sale`,`price`,`pid`  FROM ba_project WHERE time="+str(time)+" and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+"  GROUP BY name HAVING count(name) =2) ORDER BY `name`"
# sql="SELECT *  FROM ba_project WHERE time="+str(time)+" and pid="+str(id)+" and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+" and pid="+str(id)+" GROUP BY name HAVING count(name) =2) ORDER BY `name`"
# sql="SELECT *  FROM ba_project WHERE time="+str(time)+"  and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+"  GROUP BY name HAVING count(name) =2) ORDER BY `name`"
mycursor.execute(sql)
...
# sql="SELECT `name`,`sale`,`price`,`pid` FROM ba_project WHERE time ="+str(ztime)+"  and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+"   GROUP BY name HAVING count(name) =2) ORDER BY `name`"
# sql="SELECT * FROM ba_project WHERE time ="+str(ztime)+" and pid="+str(id)+" and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+" and pid="+str(id)+"  GROUP BY name HAVING count(name) =2) ORDER BY `name`"
# sql="SELECT * FROM ba_project WHERE time ="+str(ztime)+"  and name in(SELECT name FROM ba_project WHERE time >="+str(ztime)+"  GROUP BY name HAVING count(name) =2) ORDER BY `name`"
mycursor.execute(sql)
...
# # sql = "SELECT `*`, lag (`price`, 1, 0) over (ORDER BY `name`) AS tprice,lag (`sale`, 1, 0) over (ORDER BY `name`) as tsale FROM ba_project WHERE pid = 1 and time>=1666713600 and name in(SELECT name FROM ba_project WHERE time >=1666713600 and pid=1  GROUP BY name HAVING count(name) =2) "
...
# sql ="SELECT * FROM ba_project WHERE time>=1666713600 and  name in( SELECT name FROM ba_project WHERE time>=1666713600 and pid=4 GROUP BY name HAVING count(name) =1) and pid ="+str(id)+" ORDER BY name"
mycursor.execute(sql)
