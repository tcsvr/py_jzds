...
# sql = "SELECT * FROM ba_review_system WHERE id=2 "# 查询数据
# mycursor.execute(sql)
...
for x in range(2,16721):
  print(x)
  sql = "UPDATE ba_review_system SET `pl_url`='http://ls.shenzhen91.com/api/reviewsystem/index?id="+str(x)+ "' WHERE id =  " +str(x)
  # print(sql)
  mycursor.execute(sql)
  
  # mycursor.execute("UPDATE ba_review_system SET `pl_url`='is.shenzhen90.com/api/reviewsystem/index?id="+int(x)+ "' WHERE id =  " +int(x)+ ")"# 更新数据
  mydb.commit()#要有
  print(mycursor.rowcount, "record(s) affected")
